...
class SearchCompanyAdapter : RecyclerView.Adapter<SearchCompanyAdapter.ProjectHolder>() {
    ...
    override fun onBindViewHolder(holder: ProjectHolder, position: Int) {
        ...
        if (item.enProfile != null){
            Glide.with(holder.binding.root).load(ApiClient.BASE_URL_IMAGE + item.enProfile)
                .placeholder(R.drawable.ic_backround_user).into(holder.binding.ivImageProfile)
        } else {
            Glide.with(holder.binding.root).load(ApiClient.BASE_URL_IMAGE_DEFAULT_PROFILE)
                .placeholder(R.drawable.ic_backround_user).into(holder.binding.ivImageProfile)
        }
